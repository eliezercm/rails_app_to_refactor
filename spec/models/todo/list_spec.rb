  ...
  context '.call' do
    context 'without status filter' do
      it 'returns users todo list' do
        user = User.create(name: 'test', email: 'test_list@mail.com', token: SecureRandom.uuid, password_digest: Digest::SHA256.hexdigest('1234'))
    ...
    context 'with overdue status' do
      it 'returns overdue todo list' do
        user = User.create(name: 'test', email: 'test_list2@mail.com', token: SecureRandom.uuid, password_digest: Digest::SHA256.hexdigest('1234'))
